...
Summary must explain changes and uncertainties. Sources are provided separately via grounding metadata. Do not include personal identifiers in searches.'''
    request = {'systemInstruction': {'parts': [{'text': instruction}]},
               ...
               'tools': [{'googleSearch': {}}],
               # responseMimeType alone: Gemini's responseSchema dialect rejects the
               # patterns and $defs in Schedule, and the reply is validated against
               # that model on the way out anyway.
    ...
    try:
        ...
        # Worth distinguishing: grounded search needs a paid tier, and a 429 here
        # otherwise looks identical to the model simply being unavailable.
        if exc.response.status_code == 429:
            return fallback('Basic plan: Gemini is out of quota for grounded search — check billing on the API key. Opening hours have not been checked.')
